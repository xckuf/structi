...
pub async fn update_order(client: &Client, order_id: i32, updated_order: Order) -> Result<String, Error> {
    let result = client.execute(
        ...
        &[
            &order_id,
            &updated_order.car_id,
            &updated_order.customer_id,
            &updated_order.employee_id,
            &updated_order.order_date.to_string(),
            &updated_order.price,
            &updated_order.is_active
        ],
    ).await?;
...
pub async fn cancel_order(client: &Client, order_id: i32) -> Result<String, Error> {
    ...
    if result > 0 {
        Ok("\n\n\nЗаказ успешно закрыт".to_string())
    } else {
        Ok("\n\n\nЗакрытие заказа не выполнено".to_string())
    }
}
